...
func _ready() -> void:
	...
	_build_animations(GameState.selected_character_id())
...
func _build_animations(character_id: int) -> void:
	character_data = CHARACTER_CATALOG_SCRIPT.get_by_id(character_id)
	if character_data == null:
		character_data = CHARACTER_CATALOG_SCRIPT.get_by_id(1)
	if character_data == null:
		push_error("No valid default CharacterData resource is available")
		return
	sprite.sprite_frames = character_data.get_sprite_frames()

static func list_character_animation_files(folder: String) -> PackedStringArray:
	return CHARACTER_DATA_SCRIPT.list_animation_files(folder)

static func load_character_texture(path: String) -> Texture2D:
	return CHARACTER_DATA_SCRIPT.load_texture(path)
