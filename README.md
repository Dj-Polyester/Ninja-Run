...
## Scope after Phase 15

The required Main Menu, Character, Stats, Abilities, Weapons, Settings screens and expanded Level HUD are implemented. Profile actions delegate to the existing progression services and persist successful mutations immediately; weapon cards expose all task-required metadata; mobile and desktop action UI share the same logical slot/cooldown source. Phase 16 remains intentionally deferred: it is the dedicated automated-testing phase that can expand beyond the already-growing headless regression suite into the final integration/procedural stress inventory described by `task.md`. Phase 17 remains the final documentation pass; this README is kept current as phases land but does not claim final project-wide test-inventory completeness until that phase.
